...
Dataset Utilities for the Image Classification Project

This file contains the `ImagesDataset` class for loading and preprocessing images from a directory.
It also includes utility functions for image conversion to grayscale and resizing with padding.

Classes:
- `ImagesDataset`: Custom PyTorch dataset for image loading and preprocessing.

Functions:
- `to_grayscale`: Converts an image to grayscale using perceptual weighting.
- `prepare_image`: Resizes an image to specified dimensions with padding.
...
class ImagesDataset(Dataset):
    """
    Custom PyTorch dataset for loading and preprocessing images from a directory.

    Args:
        image_dir (str): Directory containing images and a CSV file with labels.
        width (int): Width to which images will be resized. Default is 100.
        height (int): Height to which images will be resized. Default is 100.
        dtype (Optional[type]): Data type for the loaded images. Default is None.
    """
    def __init__(
            self,
            image_dir,
    ...
    def load_classnames(class_filepath: str):
        """
        Loads class names and maps them to unique IDs from a CSV file.

        Args:
            class_filepath (str): Path to the CSV file containing class labels.

        Returns:
            tuple: (filenames_classnames, classnames_to_ids)
        """
    ...
    def __getitem__(self, index):
        ...
        Returns:
            tuple: (resized_image, classid, classname, image_filepath)
        """
...
def to_grayscale(pil_image: np.ndarray) -> np.ndarray:
    """
    Converts an image to grayscale using perceptual weighting.

    Args:
        pil_image (np.ndarray): Input image in RGB format.
...
def prepare_image(image: np.ndarray, width: int, height: int, x: int, y: int, size: int):
    """
    Resizes an image to the specified dimensions with padding.

    Args:
        image (np.ndarray): Input image with shape (1, H, W).
        ...
        size (int): Size of the cropped region.

    Returns:
        tuple: (resized image, cropped subarea)
    """
